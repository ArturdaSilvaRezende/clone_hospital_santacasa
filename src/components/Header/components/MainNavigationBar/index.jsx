        ...
        <div className="container mx-auto flex h-20 max-w-285 items-center justify-between">
          ...
          <nav>
            <div className="flex items-center justify-between">
              <div className="hidden w-150.5 items-center justify-between space-x-1 lg:flex">
                {Object.entries(menuData).map(([key, menu]) => (
                  <button
                    key={key}
                    onClick={() => toggleMenu(key)}
                    className={`${activeMenu === key ? 'text-[#FD0003]' : 'text-[#727070]'} group relative flex items-center gap-1 rounded-lg text-[12px] font-medium transition-colors duration-200 hover:text-[#FD0003]`}
                    aria-haspopup="true"
              ...
              <div className="container mx-auto px-4 py-4">
                {Object.entries(menuData).map(([key, menu]) => (
                  <div key={key} className="mb-2">
                    <button
                      onClick={() => toggleMenu(key)}
                      className="flex w-full items-center justify-between rounded-lg px-4 py-3 font-medium text-gray-700 transition-colors hover:bg-red-50"
                    >
                      {menu.title}
                      {/* {menu.items.length > 0 && (
                      <ChevronDown
                      ...
                      <div className="mt-2 space-y-2 pl-4">
                        {menu.items.map((item, index) => (
                          <a
                            key={index}
                            href="#"
                            className="block rounded-lg px-4 py-2 text-gray-600 transition-colors hover:bg-red-50 hover:text-red-600"
                          >
                            {item}
                          </a>
                        ))}
                ...
                <div className="mt-4 space-y-2">
                  <button className="w-full rounded-lg border border-red-600 px-4 py-2 font-medium text-red-600 transition-colors hover:bg-red-50">
                    Pré-Agendamento SUS
                  </button>
          ...
          {activeMenu &&
            ...
            !mobileMenuOpen && (
              <div
                className="animate-fadeIn absolute top-full right-0 left-0 hidden border-t border-gray-100 bg-white shadow-2xl lg:block"
                style={{
